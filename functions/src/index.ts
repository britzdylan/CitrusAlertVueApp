...
exports.sendPushNotification = onRequest(
  ...
  async (req, res) => {
    ...
    const notifToken = user.data()?.notif_token; // get from firestore
    if (!notifToken) {
      res.status(400).send("Registration token not found");
      return;
    }
    const newOrder = {...req.body} as OrderAttributes;

    // console.log(newOrder);
    if (!notifToken || !newOrder.data.attributes) {
    ...
    try {
      ...
      // update user with new message id history
      await db
        .collection("users")
        .doc(String(userId))
        .update({
